...
This code aims to dismantle networks using the Repulsion Attraction rule using GPU.

## Setting up the environment

`cd RAdismantling_supp_gpu`
...
`conda activate ra-dismantling-gpu`


You also have to install the Geometric Weights Inference package:
```bash
pip install -e ../geometric-weights-inference_supp_gpu
```

and this package:
```bash
pip install -e ../RADismantling_supp_gpu
```
## GPU
...
options:
  ...
  -H [...], --heuristics [ ...]
                        ...
                        Allowed values: ['RA2_sum', 'RA2num_sum', 'CND_sum', 'betweenness_centrality'].
...
For the reinsertion, the cpu repo needs to be used with the gpu-dismantled results file. See the cpu repo for reinsertion.
...
- `reinsertion`: whether a reinsertion step was performed at the end of the attack - disabled for GPU
- `reinsertion_type`: reinsertion method - disabled for GPU
- `reinserted_nodes`: nodes that were reinserted in the reinsertion phase - disabled for GPU
